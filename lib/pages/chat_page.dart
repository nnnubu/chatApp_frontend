...
class _ChatPageState extends State<ChatPage> with WidgetsBindingObserver {
  ...
  Future<void> _sendImageMessage({ChatItem? resendItem}) async {
    ...
    try {
      ...
      final String requestId = resendItem?.requestId ?? RequestIdGenerator.generate();
  ...
  /// 根据 requestId 更新乐观消息的 content（图片/语音上传完成后切换为网络内容）
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _isRecording
              ? (_cancelRecording ? Colors.red.shade100 : t.primaryColor.withOpacity(0.15))
              : t.thirdColor,
  ...
  Future<void> _sendVoiceMessage(String path, int seconds,
      {ChatItem? resendItem}) async {
    String? pendingRequestId;
  ...
  void _resendMessage(ChatItem item) {
    ...
    if (item.contentType == ContentType.voice.code &&
      ...
      // 本地临时文件仍在：重新上传该文件重发
      if (item.localVoicePath != null && File(item.localVoicePath!).existsSync()) {
                ...
                child: Padding(
                  ...
                  child: CommonAnimatedList(
                      ...
                      if (event is ChatListOperate) {
                        ...
                        if (event.item is ChatItem) {
                          ...
                          if (event.type == ListOperateType.insert) {
                            final bool isSelf =
                                (item.senderUid ?? item.uid) == userController.uid;
                    ...
                    deleteItemBuilder: (item, animation) {
                      if (item is! ChatItem) return const SizedBox.shrink();
                      // 删除动画：气泡向自己方向滑出屏幕 + 淡出
                      final bool isSelf =
                          (item.senderUid ?? item.uid) == userController.uid;
                      return AnimatedBuilder(
                        ...
                        builder: (context, child) {
                          ...
                          final progress =
                              Curves.easeOutCubic.transform(1 - v); // 0 -> 1
                          return Opacity(
                            opacity: v,
                  ...
                  Positioned(
                    ...
                    bottom: 12,
                    child: Obx(() {
                      final int count = _messageController.pendingNewMsgCount.value;
                    ...
                    SizedBox(
                      ...
                      child: Center(
                        child: IconButton(
                          ...
                          color: _isVoiceMode ? t.primaryColor : t.fontColor,
