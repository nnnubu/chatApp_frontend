...
- 消息撤回（软删除，撤回后双方历史不展示原文，撤回状态离线缓存同步）
- 图片 / 语音 / 视频消息（相册多选混合发送，断网可重发，语音按住说话带录音动画）
...
### AI 助手
- 与 AI 小助手聊天（后端转发 FastAPI 大模型服务）

### 本地缓存
- cached_network_image 图片缓存（首次加载后不再重复网络请求）
- Isar 业务缓存：表情包 / 好友分类 / 好友列表 / 用户资料（离线可读）
- Isar 消息表：聊天历史本地持久化（断网进入会话从缓存兜底渲染，撤回状态同步）
...
- [ ] 群聊功能
- [x] 修复会话插入消息时未隔离动画的问题
- [x] 图片/语音/视频消息（相册多选混合发送，断网可重发）
- [x] 消息撤回（软删除，撤回后双方历史不展示原文）
- [x] 消息重发与发送状态监测（本地乐观插入、ACK状态展示、失败可重发）
- [ ] 消息搜索
- [x] 读书模块框架搭建（图书列表/分类/书架/详情/搜索，epub/pdf 阅读器待接入）
- [ ] 离线推送通知（指用户不登录软件 通过系统通知消息）
- [x] 用户高频访问信息缓存本地：cached_network_image 图片缓存 + Isar 业务缓存（表情包/分类/好友/用户资料）+ Isar 消息表（聊天历史离线兜底渲染）
